...
The experiment runs ONE deterministic workload through the current DMFE
pipeline (single-pass dispatch + multi-wave full-day simulation) using the
*existing* framework, then re-aggregates the stored results by service type.
...
Determinism:
    A single documented seed drives request generation and the execution
    model, so identical-seed reruns reproduce identical core results.
...
# Workload-tuning constants (documented, not derived from nonexistent APIs).
WORKLOAD = 60          # total requests across the three services
SEED = 1404            # deterministic experiment seed
DEFAULT_MIX = {"ride": 0.40, "food": 0.40, "parcel": 0.20}
OUT_FILE = os.path.join(RESULTS_DIR, "r14_per_service.json")
...
def _run_workload(db, runner: WorkloadRunner) -> Dict[str, Any]:
...
def main() -> None:
    ...
    try:
        ...
        # Seed exactly like framework.run_workload for a reproducible baseline
        ...
        # Run the pipeline through a WorkloadRunner like the framework does.
        runner = WorkloadRunner(WORKLOAD, seed=SEED)
        runner.install_probes()
        runner.requests = reqs  # use our deterministic request set
        payload = _run_workload(db, runner)

        service = _build_per_service(db, runner, payload["all_trip_ids"])
        recon = service["reconciliation"]
        if not recon["reconciled"]:
            raise RuntimeError(
                f"Per-service totals do not reconcile: {recon}. "
                f"Refusing to write an inconsistent result.")

        overall = payload["single_pass"]["trips"]
        result = {
            "experiment": "R1.4 per-service breakdown",
            ...
            "method_notes": (
                ...
                "are attributed without double counting. Totals reconcile."),
            "limitations": (
                "One deterministic workload; synthetic requests; not a measure "
                "of real-world service fixtures."),
        }
        ...
        print("\n=== R1.4 Per-Service Breakdown ===")
        print(f"seed={SEED} workload={WORKLOAD} mode={result['mode']}")
        for s in SERVICES:
            row = result["per_service"][s]
            print(f"  {s:7s}: reqs={row['requests']} dispatched={row['dispatched_requests']} "
                  f"assign%={row['assignment_rate_pct']} trips={row['trips']} "
                  f"shared={row['shared_trips']} batch%={row['batching_rate_pct']}")
        print(f"  reconciliation OK: {recon['reconciled']}")
