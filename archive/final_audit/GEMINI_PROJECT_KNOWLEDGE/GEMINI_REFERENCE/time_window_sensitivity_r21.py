...
--------------------------------
    1. For each sweep value, seed the isolated evaluation SystemConfig with
       ``max_allowed_delay_min = <value>`` and call the project's own
       ``clear_config_cache()`` so the new value is picked up from the DB
       (config is TTL-cached).
    2. Regenerate a deterministic request set for that sweep (same seed,
       identical request pool across sweep points so only the knob changes).
...
OUT_FILE = os.path.join(RESULTS_DIR, "r21_time_window.json")


def run_sweep_point(value: int, seed: int) -> Dict[str, Any]:
    exp = ExperimentDB()
    db = SessionLocal()
    try:
        exp.reset_schema()
        ...
        if len(reqs) < WORKLOAD:
            raise RuntimeError(
                f"Sweep {value} got {len(reqs)}/{WORKLOAD} requests; aborting "
                f"this sweep point (no fabricated rows).")
...
def main() -> None:
    ...
    for v in SWEEP_MIN:
        results[str(v)] = run_sweep_point(v, SEED)

    all_took_effect = all(p["knob_took_effect"] for p in results.values())
    ...
    result = {
        ...
        "points": results,
        "method_notes": (
            "Each sweep point reseeds SystemConfig.max_allowed_delay_min in "
            "the isolated evaluation DB and calls clear_config_cache() so the "
            "new value is actually read.  Requests are regenerated with the "
            "same deterministic seed at every sweep point, so only the "
            "time-window knob changes between points.  The engine's read-back "
            "of the knob is recorded per point."),
        "limitations": (
